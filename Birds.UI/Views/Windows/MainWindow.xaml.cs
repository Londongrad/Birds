...
public partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        Loaded += (_, _) =>
        {
            ...
            UpdateQuickOperationIndicatorState(false);
            UpdatePendingDeleteUndoState(false);
    ...
    {
        if (MaximizeGlyph == null || RestoreGlyph == null) return;
    ...
    private void OnViewModelPropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        if (e.PropertyName is nameof(MainViewModel.HasRecentOperationStatus)
            ...
            UpdateQuickOperationIndicatorState(true);

        if (e.PropertyName == nameof(MainViewModel.IsNotificationCenterOpen)) UpdateAdaptiveMinWidth();

        if (e.PropertyName == nameof(MainViewModel.IsArchiveViewActive)) UpdateAdaptiveMinWidth();

        if (e.PropertyName is nameof(MainViewModel.HasPendingDeleteUndo)
            or nameof(MainViewModel.PendingDeleteUndoVersion))
            UpdatePendingDeleteUndoState(true);
    }
    ...
    private void AnimateQuickOperationIndicatorOut()
    {
        ...
        if (TryGetQuickIndicatorTransforms(out var scale, out var translate))
        {
            scale.BeginAnimation(ScaleTransform.ScaleXProperty,
                new DoubleAnimation(0.82, TimeSpan.FromMilliseconds(220)));
            scale.BeginAnimation(ScaleTransform.ScaleYProperty,
                new DoubleAnimation(0.82, TimeSpan.FromMilliseconds(220)));
            translate.BeginAnimation(TranslateTransform.YProperty,
                new DoubleAnimation(-2, TimeSpan.FromMilliseconds(220)));
        }
    }
    ...
    private void AnimateQuickOperationPulse()
    {
        ...
        pulseScale.BeginAnimation(ScaleTransform.ScaleXProperty,
            new DoubleAnimation(1.85, TimeSpan.FromMilliseconds(540))
            {
                From = 0.8,
                BeginTime = TimeSpan.FromMilliseconds(40),
                EasingFunction = new QuadraticEase { EasingMode = EasingMode.EaseOut }
        ...
        pulseScale.BeginAnimation(ScaleTransform.ScaleYProperty,
            new DoubleAnimation(1.85, TimeSpan.FromMilliseconds(540))
            {
                From = 0.8,
                BeginTime = TimeSpan.FromMilliseconds(40),
    ...
    private void UpdateAdaptiveMinWidth()
    {
        ...
        if (WindowState == WindowState.Normal && Width < targetMinWidth) Width = targetMinWidth;
    }
    ...
    private void AnimatePendingDeleteUndoProgress(TimeSpan duration)
    {
        ...
        if (PendingDeleteUndoProgressBar is not null)
        {
            ...
            PendingDeleteUndoProgressBar.BeginAnimation(OpacityProperty,
                new DoubleAnimation(0.72, TimeSpan.FromMilliseconds(420))
                {
                    AutoReverse = true,
    ...
    private static Color ResolveUndoProgressMidColor()
    {
        return Color.FromRgb(214, 171, 89);
    }
