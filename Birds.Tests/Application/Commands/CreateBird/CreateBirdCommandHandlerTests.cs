...
namespace Birds.Tests.Application.Commands.CreateBird
{
    public class CreateBirdCommandHandlerTests
    {
        ...
        public async Task Handle_Should_Return_SuccessResult_When_Command_Is_Valid()
        {
            ...
            var expectedBird = Bird.Create(command.Name, command.Description, command.Arrival, command.Departure, command.IsAlive);
            ...
            result.Value.Should().BeEquivalentTo(expectedDto, options => options.Excluding(dto => dto.Id).Excluding(dto => dto.CreatedAt));
